...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (ok) setTerminal(u?.terminal || null);
      } catch {
  ...
  return (
    <div className="h-screen flex flex-col font-sans">
      <header className="bg-[#C14600] flex items-center justify-between px-3">
        {/* Izquierda: badge de terminal */}
        <div className="flex items-center gap-2 py-2">
          <span className="text-white font-semibold">Yajalones</span>
          {terminal && (
            <span className="text-[#C14600] bg-white/90 px-2 py-0.5 rounded text-xs font-medium">
              {terminal === 'YAJALON' ? 'Yajalón' : terminal === 'TUXTLA' ? 'Tuxtla' : terminal}
            </span>
          )}
        </div>
        ...
        <nav className="flex">
          ...
          <button
            ...
            className="text-white text-sm px-4 hover:text-gray-200 font-inter cursor-pointer"
          >
            Cerrar sesión
          </button>
        </nav>
